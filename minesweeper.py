...
class Board:
    def __init__(self, dim_size, num_bombs):
        ...
        # intialize a set to keep track of which location we're uncovered
        #we'll save (row,col) takes into this set
    ...
    def make_new_board(self):
        ...
        # [...                  ],
        # [None, None, ..., None],
        # we can see now this represents a board!
        ...
        while bombs_planted < self.num_bombs:
            ...
            if board[row][col] == '*':
                #this means we've actually planted a bomb there alreadly so keep going
                continue
    ...
    def get_num_neighboring_bombs(self, row, col):
        ...
        for r in range(max(0, row-1), min(self.dim_size-1, row+1)+1):
            for c in range(max(0, col-1), min(self.dim_size-1, col+1)+1):
                if r == row and c == col:
                    # our against location, don't check
                    continue
                if self.board[r][c] == '*':
    ...
    def __str__(self):
        ...
        # it'll print out what this fuction returns!
...
def play(dim_size=10, num_bombs=10):
    ...
    #step3a: if the location is the bomb, show game over message
    #step3b: if location is not a bomb, dig recursively until each square is
